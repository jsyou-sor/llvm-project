...
void __assert_fail_real(const char *expr, const char *file, unsigned int line, const char *func);
//void __assert_fail_real(const char *expr, const char *file);
...
//void __assert_fail(const char *expr, const char *file) {
    ...
    return __assert_fail_real(expr, file, line, func);
		//return __assert_fail_real(expr, file);
}
...
void vsyslog(int priority, const char *message, va_list ap) {
		void *tmp;
		//ap = __sgxbounds_uninstrument((void*)ap);
		tmp = __sgxbounds_uninstrument((void *)ap);
		ap = tmp;
		message = __sgxbounds_uninstrument(message);
    vsyslog_real(priority, message, ap);
}
