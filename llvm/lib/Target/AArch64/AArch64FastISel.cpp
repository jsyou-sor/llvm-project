...
unsigned AArch64FastISel::emitAdd_ri_zt(MVT VT, unsigned Op0, bool Op0IsKill,
  ...
  if (Imm < 0)
    ResultReg = emitAddSub_ri_zt(false, VT, Op0, Op0IsKill, -Imm, false, true, ZTData);
  else
  ...
  ResultReg = emitAddSub_rr_zt(true, VT, Op0, Op0IsKill, CReg, true, false, true, ZTData);
...
bool AArch64FastISel::selectGetElementPtr(const Instruction *I) {
  ...
  for (gep_type_iterator GTI = gep_type_begin(I), E = gep_type_end(I);
    ...
    if (auto *StTy = GTI.getStructTypeOrNull()) {
      ...
      N = emitAddSub_rr_zt(true, VT, N, NIsKill, IdxN, IdxNIsKill, false, true, ZTData);
      //errs() << "[AArch64FastISel::selectGetElementPtr]\tfastEmit_rr ret N = " << N << " ret M = " << M << "\n";
      if (!N)
        return false;
    }
  }
