...
#define GETSGXBOUNDSFUNC(NAME)  { if (F->getName().equals(#NAME)) NAME = F; }
	...
	void visitGlobals() {
		...
		for (size_t i = 0; i < n; i++) {
			...
			if (__sgxbounds_init->getEntryBlock().getFirstNonPHI() == NULL)
				errs() << "!!\n";
	    IRBuilder<> IRBInit(__sgxbounds_init->getEntryBlock().getFirstNonPHI());
			//IRBuilder<> IRBInit(__sgxbounds_init->getEntryBlock());
...
public:
	...
	SgxBoundsModule(): ModulePass(ID) {
		initializeSgxBoundsModulePass(*PassRegistry::getPassRegistry());
	}
	StringRef getPassName() const override { return "SgxBoundsModule"; }
	virtual bool runOnModule(Module &M) override {
		//DEBUG(dbgs() << "[RUNNING PASS: sgxbounds]\n");
		errs() << "[RUNNING PASS: sgxbounds]\n";
...
char SgxBoundsModule::ID = 0;
INITIALIZE_PASS_BEGIN(SgxBoundsModule, "sgxbounds-module",
	"SgxBounds: detecs..."
	"ModulePass",
	false, false)
INITIALIZE_PASS_DEPENDENCY(ScalarEvolutionWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_END(SgxBoundsModule, "sgxbounds-module",
	"SgxBounds: detects..."
	"ModulePass",
	false, false)
ModulePass *llvm::createSgxBoundsModulePass() {
	return new SgxBoundsModule();
}
