...
#define LOWFAT_NUM_REGIONS 58
#define LOWFAT_SIZES_PAGES 1
#define LOWFAT_REGION_SIZE _LOWFAT_REGION_SIZE
#define LOWFAT_REGION_SIZE_SHIFT 32
#define LOWFAT_STACK_MEMORY_SIZE 1073741824
#define LOWFAT_GLOBAL_MEMORY_SIZE 1073741824
#define LOWFAT_HEAP_MEMORY_SIZE 2147418112
#define LOWFAT_STACK_MEMORY_OFFSET 3221225472
#define LOWFAT_GLOBAL_MEMORY_OFFSET 2147446784
...
#define LOWFAT_MAX_HEAP_ALLOC_SIZE 1073741824
...
const LOWFAT_CONST_DATA ssize_t lowfat_stack_offsets[64+1] =
{
	...
	0, /* idx=37 */
	0, /* idx=38 */
	-25769803776,	/* idx=39, size=33554432 */
	-30064771072,	/* idx=40, size=16777216 */
	-34359738368,	/* idx=41, size=8388608 */
	-38654705664,	/* idx=42, size=4194304 */
	-42949672960,	/* idx=43, size=2097152 */
	-47244640256,	/* idx=44, size=1048576 */
	-51539607552,	/* idx=45, size=524288 */
	-55834574848,	/* idx=46, size=262144 */
	-60129542144,	/* idx=47, size=131072 */
	-64424509440,	/* idx=48, size=65536 */
	-68719476736,	/* idx=49, size=32768 */
	-73014444032,	/* idx=50, size=16384 */
	-90194313216,	/* idx=51, size=8192 */
	-111669149696,	/* idx=52, size=4096 */
	-133143986176,	/* idx=53, size=2048 */
	-154618822656,	/* idx=54, size=1024 */
	-176093659136,	/* idx=55, size=512 */
	-197568495616,	/* idx=56, size=256 */
	-219043332096,	/* idx=57, size=128 */
	-236223201280,	/* idx=58, size=64 */
	-244813135872,	/* idx=59, size=32 */
	-249108103168,	/* idx=60, size=16 */
	-249108103168,	/* idx=61, size=16 */
	-249108103168,	/* idx=62, size=16 */
	-249108103168,	/* idx=63, size=16 */
	-249108103168,	/* idx=64, size=16 */
};
	...
	switch (__builtin_clzll(size))
	{
		...
		case 37:
			if (size <= 67108864-1-0)
				return 54;
		case 36:
			if (size <= 134217728-1-1)
				return 55;
		case 35:
			if (size <= 268435456-1-3)
				return 56;
		case 34:
			if (size <= 536870912-1-7)
				return 57;
		case 33:
			if (size <= 1073741824-1-14)
				return 58;
		case 32:
		case 31:
		case 30:
